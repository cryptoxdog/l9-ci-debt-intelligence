...
**Upstream:** `cryptoxdog/l9-ci-debt-resolver` emits `PR_REMEDIATION_FINDINGS.jsonl` and triggers this repo via `repository_dispatch` event `corpus-update`.
**Downstream:** `cryptoxdog/l9-ci-debt-lsp` consumes compiled rules published by `publish-astgrep-pack` as a GitHub release artifact.
...
Listens for `repository_dispatch` event `corpus-update`.
`corpus-update.yml` reads `client_payload.findings_path` and fetches the artifact from the resolver run.
